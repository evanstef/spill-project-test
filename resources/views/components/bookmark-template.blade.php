...
<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        ...
        <div>
            <p class="text-xs sm:text-base">{{ $bookmark->post->created_at->diffForHumans() }}</p>
        </div>
    </div>
    ...
    {{-- Post Image --}}
    @if ($bookmark->post->images->count() > 0)
        ...
        <div id="carousel-bookmark-{{ $bookmark->post->id }}" class="carousel carousel-bookmark w-full relative" data-post-bookmark-id="{{ $bookmark->post->id }}">
            ...
            {{-- Arrow Controls --}}
            <button data-post-bookmark-id="{{ $bookmark->post->id }}" class="prevArrowBookmark absolute left-2 xl:left-5 top-1/2 transform -translate-y-1/2 cursor-pointer">
            ...
            <button data-post-bookmark-id="{{ $bookmark->post->id }}" class="nextArrowBookmark absolute right-2 xl:right-5 top-1/2 transform -translate-y-1/2 cursor-pointer">
        ...
        {{-- Dots Indicator --}}
        <div id="dots-bookmark-{{ $bookmark->post->id }}" class="flex justify-center mt-2 gap-1" data-post-bookmark-id="{{ $bookmark->post->id }}">
            @foreach ($bookmark->post->images as $key => $image)
                <div class="dot dot-bookmark w-1 h-1 xl:w-2 xl:h-2 rounded-full {{ $key === 0 ? 'bg-gray-600' : 'bg-gray-400' }}" data-dot-index="{{ $key }}"></div>
            @endforeach
        </div>
        @endif
    @endif
    ...
    <div class="flex justify-around items-center">
        ...
        <form action="{{ Auth::check() && $bookmark->post->likedByUsers->contains(Auth::user()->id) ? route('post.unlike', $bookmark->post) : route('post.like', $bookmark->post) }}" method="POST">
            ...
            <div x-data="{ showWhoLikesPostBookmarks : false}" class="flex items-center gap-2">
                ...
                <div x-show="showWhoLikesPostBookmarks"
                    ...
                    class="fixed w-full flex justify-center items-center h-full top-0 left-0 right-0 z-50 bg-gray-900/70" style="display: none;">
                    <div class="w-[65%] sm:w-[60%] lg:w-[35%] xl:w-[25%] h-[55%] sm:h-[50%] md:h-[60%] dark:bg-gray-700 rounded-lg p-2 sm:p-4 xl:p-6 relative">
                        ...
                        <button type="button" @click="showWhoLikesPostBookmarks = false" id="hide-followers" class="absolute top-1 right-1">
                        ...
                        <p class="font-bold text-sm sm:text-base md:text-lg text-center">Total Likes This Post {{ $bookmark->post->likedByUsers->count() }}</p>
                        <div class="w-full h-0.5 my-4 bg-gray-200"></div>

                        {{-- menampilkan user yang like --}}
                        <div class="space-y-5 h-[85%] {{ $userLikesPost->count() >= 8 ? 'overflow-y-scroll' : '' }} custom-scrollbar">
        ...
        {{-- comment --}}
        <a class="flex items-center gap-2" href="{{ route('post.show', $bookmark->post) }}">
